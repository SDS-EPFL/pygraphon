...
class StepGraphon:
    ...
    def draw(
        self, rho: float, numberNodes: int = None, exchangeable: bool = False
    ) -> np.ndarray:
    ...
    def approximateGraphonFromAdjacency(
        ...
        if bandwidthHist is None:
            ...
            idx = matlabEngine.nethist(npArray2Matlab(
                adjacencyMatrix), argh, nargout=1)
    ...
    def distance(self, stepGraphon, norm: str = "MISE") -> float:
        ...
        if len(np.unique(self.graphon.shape)) != 1 or len(np.unique(stepGraphon.graphon.shape)) != 1:
        ...
        for sigma in permutations_possible:
            if norm == "MISE":
                result = np.sqrt(np.sum(((graphon1 - permute_matrix(graphon2,sigma)) ** 2) * self.areas))
    ...
    def norm(self, norm: str = "MISE"):
        empty = StepGraphon(
            graphon=np.zeros_like(self.graphon), bandwidthHist=self.bandwidthHist
        )
    ...
    def get_edge_probabilities(self, numberNodes: int = None, latentVarArray: np.ndarray = None, exchangeable: bool = False, wholeMatrix: bool = True) -> np.ndarray:
        ...
        elif numberNodes is not None or (self.EdgeProbabilities is None and self.numberNodes is not None):
        ...
        for index, nodes in enumerate(zip(I, J)):
            probs[index] = self.graphon_function(latentVarArray[nodes[0]],
                                                 latentVarArray[nodes[1]])
