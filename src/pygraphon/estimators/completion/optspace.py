...
class OptSpace(object):
    """
    Optsace is a python eqquivalent of the R package OptSpace.
    ...
    def solve(self, obs):
        ...
        # raise future warning if hard set
            ...
            if self.n_components > (min(n, m) - 1):
                raise ValueError(
                    "n-components must be at most" " 1 minus the min. shape of the" " input matrix."
                )
        # otherwise rase an error.
        else:
            raise ValueError("n-components must be " "an interger or 'auto'")
        ...
        dist[0] = norm(np.multiply(obs_error, mask), "fro") / np.sqrt(total_nonzeros)
        # we will perform gradient decent for at most self.max_iterations
        for i in range(1, self.max_iterations):
            # generate new optimized loadings from F(X,Y) [1]
            U_update, V_update = gradient_decent(U, V, S, obs, mask, self.step_size, rho)
            ...
            dist[i + 1] = norm(np.multiply(obs_error, mask), "fro") / np.sqrt(total_nonzeros)
...
def cost_function(U, V, S, obs, mask, step_size, rho):
    ...
    distortion = np.sum(np.sum((np.multiply((U.dot(S).dot(V.T) - obs), mask) ** 2))) / 2
...
def gradient_decent(U, V, S, obs, mask, step_size, rho):
    ...
    U_update = (
        np.multiply((imputed - obs), mask).dot(VS)
        + U.dot(Qu)
        + rho * grassmann_manifold_two(U, step_size, n_components)
    )
    V_update = (
        np.multiply((imputed - obs), mask).T.dot(US)
        + V.dot(Qv)
...
def rank_estimate(obs, eps, k=20, lam=0.05, min_rank=3, max_iter=5000):
    """
    ...
    s = svds(obs, min(k, n, m) - 1, which="LM", return_singular_vectors=False)[::-1]
