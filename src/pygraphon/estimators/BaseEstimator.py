...
class BaseEstimator(ABC):
    ...
    def estimate(self, graph: nx.Graph = None,
                 adjacency_matrix: np.ndarray = None) -> Graphon:
        ...
        if graph is not None and adjacency_matrix is not None:
            raise ValueError(
                "graph or adjacency_matrix must be provided, not both")
    ...
    def _approximateGraphonFromAdjacency(
            self, adjacency_matrix: np.ndarray) -> Graphon:
